...
if( $_POST['option'] == "add" )
{
	try
	{
		...
		// if everything is ok, try to upload file
			...
			if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_dir . $bkid . "." . $imageFileType)) {
...
if( $_POST['option'] == "upebook" )
{
	try
	{
		...
		$stmt = $conn->prepare("INSERT INTO EBOOKS (EBK_TITLE,EBK_AUTHOR,EBK_GEN,EBK_SYN) VALUES(?,?,?,?) ");
		$stmt->execute(array($btitle,$bauthor,$bgenre,$bsyn));
		$bkid = $conn->lastInsertId();
...
// End  List all ebook list

// Start delete multiple ebooks items
...
// Delete delete multiple ebooks items
	  ...
	  <div class='box-body'>
		...
		<div class='form-group'>
		  <label>Choose a photo to upload:</label>
				<input type='file' name='fileToUploadUpd' id='fileToUploadUpd' class='form-control'>
				"; 
...
if($_POST['option'] == "updatedataModal")
{
	...
	try
	{
		...
		//upload image
		$target_dir = "../images/bookscover/";
		$target_file = $target_dir . basename($_FILES["fileToUploadUpd"]["name"]);
		...
		// Check file size
		if ($_FILES["fileToUploadUpd"]["size"] > 500000) {
			//echo "Sorry, your file is too large.";
			$uploadOk = 0;
		}
		// Allow certain file formats
		...
		// if everything is ok, try to upload file
			...
			if (move_uploaded_file($_FILES["fileToUploadUpd"]["tmp_name"], $target_dir . $bk_id . "." . $imageFileType)) {
				echo "<script>alertify.success('The image has been uploaded.')</script>";
				$imgstat = 1;
			} else {
				echo "<script>alertify.error('Sorry, there was an error uploading your file.')</script>";
